...
/**
 * Show the message for a conversion script that timed out.
 * 
 * @since 2.0
 * @return {void}
 */
...
function convertCptCount() {
    ...
    $.ajaxQueue({
        ...
        if ( response === -1 || response === '-1' ) {
            ...
            /* A count that stopped moving means the convert script died
             * rather than finished, so that gets the timeout message.
             */
...
/* Copy the remaining conversion count into the thickbox field. */
if ( $( ".error .wpsl-cpt-remaining" ).length ) {
    let cptCount = parseInt( $( ".error .wpsl-cpt-remaining" ).first().text() );
